...
let RestaurantService = class RestaurantService {
    constructor(restaurantRepository, clientRepository) {
    ...
    async create(restaurantDto) {
        ...
        if (clients != null && clients.length > restaurantDto.capacity) {
            throw new common_1.ForbiddenException("Restaurant is full of capacity");
        }
        const restaurant = this.restaurantRepository.create({ ...restaurant_dto_1.RestaurantDto, clients: clients, name: name, address: address, capacity: capacity || 1 });
    ...
    async update(id, updateRestaurantDto) {
        ...
        if (updateRestaurantDto.clients && updateRestaurantDto.clients.length > 0) {
            const clientEntities = await this.clientRepository.find({
                where: { id: (0, typeorm_2.In)(updateRestaurantDto.clients) },
            });
            if (clientEntities.length !== updateRestaurantDto.clients.length) {
                throw new common_1.NotFoundException("Some clients were not found");
            }
    ...
    async removeClient(clientId, restaurantId) {
        ...
        if (clientIndex === -1) {
            throw new common_1.NotFoundException("Client not found in the restaurant");
        }
